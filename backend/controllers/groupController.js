...
async function gravarAuditoria(
  ...
  try {
    ...
    await logErro("GROUP_CONTROLLER_GRAVAR_AUDITORIA", e);
  }
}
...
exports.getAllGroups = async (req, res) => {
  try {
    ...
    await logErro("GROUP_CONTROLLER_GET_ALL", error);
...
exports.createGroup = async (req, res) => {
  ...
  try {
    ...
    await logErro("GROUP_CONTROLLER_CREATE", error);
...
exports.updateGroup = async (req, res) => {
  ...
  try {
    ...
    await connection.rollback();
    await logErro("GROUP_CONTROLLER_UPDATE", error);
...
exports.deleteGroup = async (req, res) => {
  ...
  try {
    ...
    await connection.rollback();
    await logErro("GROUP_CONTROLLER_DELETE", error);
    res.status(500).json({ error: error.message });
  } finally {
    connection.release();
  }
...
exports.getUserGroups = async (req, res) => {
  ...
  try {
    const [rows] = await db.execute(
      `SELECT g.id AS grupo_id, g.nome AS grupo_nome FROM usuarios u LEFT JOIN grupos g ON u.grupo_id = g.id WHERE u.id = ?`,
      [userId],
    );
    res.json(rows);
  } catch (error) {
    await logErro("GROUP_CONTROLLER_GET_USER_GROUPS", error);
