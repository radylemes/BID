...
function gerarTokenApp(user) {
  const userRole = user.perfil || user.role || (user.is_ad_user ? "user" : "admin");
  return jwt.sign({ id: user.id, role: userRole }, JWT_SECRET, { expiresIn: "8h" });
}

// ============================================================================
// MOTOR DE AUDITORIA (HELPER)
// ============================================================================
async function registrarLogLogin(usuarioId, metodo, emailOuUsername) {
  try {
    await db.execute(
      ...
      [usuarioId, usuarioId, JSON.stringify({ metodo, identificador: emailOuUsername })]
    );
  } catch (e) {
    console.error("Erro ao registrar auditoria de login:", e.message);
  }
}
...
exports.login = async (req, res) => {
  try {
    ...
    // Traz a árvore completa: Empresa/Setor (AD) e Grupo (Apostas)
    ...
    if (user.is_ad_user === 1 && !user.senha_hash) return res.status(400).json({ message: 'Use "Entrar com Microsoft".' });
    if (!user.senha_hash) return res.status(401).json({ message: "Senha não definida." });
    ...
    res.json({
      auth: true, token: gerarTokenApp(user),
      user: {
        id: user.id, username: user.username, nome_completo: user.nome_completo,
        email: user.email, 
        setor: user.setor_nome || "Geral", 
        empresa: user.empresa_nome || "Geral", 
        grupo_nome: user.grupo_nome || "Sem Grupo",
        foto: user.foto, role: user.perfil, pontos: user.pontos
      }
    });
...
exports.loginMicrosoft = async (req, res) => {
  ...
  try {
    ...
    if (!userLocal) {
      ...
      if (uMail.length > 0) {
        userLocal = uMail[0];
        try { await db.execute("UPDATE usuarios SET microsoft_id = ?, is_ad_user = 1 WHERE id = ?", [oid, userLocal.id]); } catch (err) {}
      }
    }
    ...
    if (!userLocal) {
      ...
      const [newUsers] = await db.execute("SELECT * FROM usuarios WHERE id = ?", [result.insertId]);
    ...
    res.json({
      auth: true, token: gerarTokenApp(userLocal),
      user: {
        id: userLocal.id, username: userLocal.username, nome_completo: userLocal.nome_completo,
        email: userLocal.email, 
        setor: userLocal.setor_nome || "Geral", 
        empresa: userLocal.empresa_nome || "Geral", 
        grupo_nome: userLocal.grupo_nome || "Sem Grupo",
        foto: userLocal.foto, role: userLocal.perfil, pontos: userLocal.pontos
      }
    });
